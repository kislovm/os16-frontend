...
AN = function() {
        ...
        new_order_edit: function() {
            ...
            $.post('/order/new/', function(data) {
                ...
                that.ordeforming = new OrderformingView({
                    el: element,
                    model: new OrderformingModel({ orderId: orderId })
			    });
    ...
    var ProductNamingView = Backbone.View.extend({
        ...
        _fetchData: function() {
            var loading =
                this.$el.find('.product-naming__loading').show();
        ...
        _uploadFile: function() {
            ...
            this.$el.find('.product-naming__xls-form').ajaxSubmit({
                success: function(data) {
                    ...
                    else _this.collection.set(data);
                },
                dataType: 'json'
            });
        },
        ...
        _save: function() {
            if(confirm('Внимание! Редактирование будет заблокировано, а таблица отправлена на утверждение менеджеру. ' +
                ...
                $.post('/settings/prodnames/load/', { products: JSON.stringify(this.collection.toJSON()) }, function(data) {
                    console.log(data);
                }, 'json')
...
//                $el.find('.product-naming__controls').html('');
        ...
        _selectPosition: function(e) {
            ...
            this._popup = new Popup({ contentView: SelectPositionView, contentModel: new SelectPositionModel(), contentParams: { itemModel: this.collection.get(el.data('model-id')) } });
        },
        ...
        _addRow: function(customerName, anName, cantDelete) {
            this.$el.find('table').append(
                '<tr>' +
                    '<td>' + customerName + '</td>' +
                    (anName ? '<td>' + anName + '</td>' : '<td><span class="product-naming__select-position">Выбрать</span></td>') +
                    (cantDelete && '<td class="product-naming__remove-cell"><span class="product-naming__remove-row">Удалить</span></td>') +
                '</tr>')
        },
        ...
        set: function(key, value, options) {
            ...
            for (attr in attrs) {
                if (attr == 'call_duration') {
                    ...
                    attrs['call_duration'] = callDuration == 0 ? callDuration + ' секунд' :
	...
	var OrderView = ANView.extend({ //View элемента коллекции. 
		_template: JST['orders/order'],
		//Его template в файле jst.js
...
*/
	...
	var OrderformingView = ANView.extend({
		...
		events: {
			"click .add ": "addViewOpen",
			//"click #orderlistcontent tr:not(.add)": "selectItem",
        ...
        changeAddView: function() {
			...
			if (amount = this.item.get('amount')) {
				var that = this;
				$.post('/order/' + this.model.get('orderId') + '/add/' + this.item.get('itemId') + '/',
                    { amount: amount },
                    function(data) {
                        if (data.error) {
                        ...
                        if (that.model.get('orderId') == 0 && data.orderId != undefined && that.model.get('orderId') != data.orderId) window.location.hash = '#order/' + data.orderId
                        else that.model.fetch() && alert('Товар добавлен');
                }, 'json');
			} else if (this.item.get('itemId') != undefined) {
				this.addView_ = new OrderformingAddView__({
					item: this.item,
					model: new ItemHistoryModel({
                ...
                this.addView_ = new OrderformingAddView({
                    item: this.item,
					model: new GroupListModel({ searchString: "Поиск группы товаров", categoryId: this.item.get('category') })
                });
                $('#leftpage', $(this.el)).append($(this.addView_.el));
            } else {
                this.addView_ = new OrderformingAddView___({
                    item: this.item,
					model: new CategoryListModel({ searchString: "Поиск категории товаров" })
                });
                $('#leftpage', $(this.el)).append($(this.addView_.el));
            }
        ...
        amountChange: function(item) {
            ...
            $.post(this._amountChangeUrl(item.id),
                { amount: item.amount },
            'json');
        ...
        flash: function(item) {
            var tr = item.el.parents('tr').addClass('flash'),
                icons = item.el.parents('tr').find('.icon').addClass('icon_disabled_yes'),
                input = item.el.parents('tr').find('.amount').prop('disabled', true);
            setTimeout(function() { alert('Количество товара изменено'); }, 2000)
            setTimeout(function() { tr.removeClass('flash'); icons.removeClass('icon_disabled_yes'); input.prop('disabled', false) }, 2100);
            $.get('/order/' + this.model.get('orderId') + '/', function(data) {
		...
		render: function() {
            ...
            items.sort(function(a, b) {
                if(that.sortRule != 'name' && that.sortRule != 'special') {
                    a[that.sortRule] && a[that.sortRule].replace && (a[that.sortRule] = +a[that.sortRule].replace(',', '.'));
                    b[that.sortRule] && b[that.sortRule].replace && (b[that.sortRule] = +b[that.sortRule].replace(',', '.'));
                }
...
//			}
    ...
    var Popup = ANView.extend({
        ...
        render: function() {
            ...
            this.contentModel ?
                new this.contentView({ el: popup.find('.popup__content'), model: this.contentModel, params: this.contentParams }) :
    ...
    var ItemAdditionalInfoView = ANView.extend({
        ...
        render: function() {
            ...
            this.$el.html(
                '<h1 class="popup__header">Дополнительная информация о товаре</h1>' +
                    '<div>Остаток по складу: '+ (json.stock_amount || 'Нет') +'</div>' +
                    '<div>В дороге: '+ (json.amount_in_way || 'Нет') +'</div>' +
                    '<div>Скорость продаж: '+ json.average + ' ' + json.ed_izm + ' в месяц</div>' +
                    '<div>Кратность коробок: ' + json.amount_in_pack + '</div>' +
                    '<div>Краткое описание применения: '+ json.product_application +'</div>'
            );
        }
    ...
    var Backorder = ANView.extend({
        ...
        onSubmit: function(e) {
            ...
            this.$el.find('form').ajaxSubmit({ success: function(data) {
                if(data.error){
                ...
                GLOBAL.trigger('closePopups');
                setTimeout(function() { controller.navigate('new_order/edit', { trigger: true }); alert("Заказ создан на основе загруженных остатков") }, 500);
            }, dataType: 'json'});
	...
	var OrderformingAddView = PopupView.extend({
		...
		submitForm: function() {
			this.selectedItem != undefined &&
				this.options.item.set('group', this.selectedItem) &&
				    this.close()
		},
	...
	var OrderformingAddView_ = OrderformingAddView.extend({
		...
		inputHelp: "Поиск по названию товара",
	...
	var OrderformingAddView__ = OrderformingAddView.extend({
		template: JST['orderforming/add2'],
		defaultInput: "#quontity-input",
        back: function(e) {
            e.preventDefault();
    ...
    var OrderformingAddView___ = OrderformingAddView.extend({
		...
		submitForm: function() {
			this.selectedItem != undefined &&
				this.options.item.set('category', this.selectedItem) &&
				    this.close()
		}
